...
public class Main {
	...
	// Query Folder -- contains SPARQL queries that is used to extract
	// suggestions.
	...
	public static List<String> suggestion= new ArrayList<String>();
	public static String deduceValue;
	...
	public static void processdata() {
		try {
			...
			ReadFile.enrichJenaModelOntologyDataset(model,
					GENERATED_SEMANTIC_SENSOR_DATA);

			// works with semantic sensor data
			// (file already generated, useful to use in case of some issues
			// with the semantic annotation)
			// ReadFile.enrichJenaModelOntologyDataset(model,
			// HEALTH_SEMANTIC_SENSOR_DATA);

			// GENERIC APPLICATION
			GenericApplication generic_appli = new GenericApplication(model);

			// STEP: SPECIFIC DOMAIN ONTOLOGIES AND DATASETS
			ReadFile.enrichJenaModelOntologyDataset(generic_appli.model,
					M3_ONTOLOGY);
			ReadFile.enrichJenaModelOntologyDataset(generic_appli.model,
					NATUROPATHY_ONTOLOGY);
			ReadFile.enrichJenaModelOntologyDataset(generic_appli.model,
					NATUROPATHY_DATASET);
			ReadFile.enrichJenaModelOntologyDataset(generic_appli.model,
					HEALTH_ONTOLOGY);
			...
			Model deduceMeaningfulInformationFromSensorData = generic_appli
					.executeReasoningEngine(LINKED_OPEN_RULES_HEALTH);

			// deduceMeaningfulInformationFromSensorData.write(System.out);

			// STEP: TO MODIFY THE GENERIC SPARQL QUERY BY ASKING SPECIFIC
			// INFORMATION
			ArrayList<VariableSparql> var = new ArrayList<VariableSparql>();
			var.add(new VariableSparql("inferTypeUri", NAMESPACE_M3
					+ "BodyTemperature", false));// we look for body temperature
													// measurement

			// STEP: EXECUTING QUERY ENGINE
			ExecuteQueryEngine resultQueryEngine = new ExecuteQueryEngine(
					deduceMeaningfulInformationFromSensorData,
					SPARQL_QUERY_NATUROPATHY);
...
//			List<String> suggestion = new ArrayList<String>();
			...
			suggestion.remove(0);
			deduceInfo.remove(0);
			deduceValue=deduceInfo.get(0);
			System.out.println("suggestion is " + suggestion + "deduceInfo is "
					+ deduceInfo);
			System.out.println("deduce value is "+deduceValue);
			 ...
			 * DriverManager.getConnection( "jdbc:mysql://localhost/swot",
			 * USERNAME, PASSWORD);
			 * 
			 * statement = connect.createStatement();
			 * 
			 * while (suggestionIterator.hasNext() &&
			 * deduceInfoIterator.hasNext()) {
			 * 
			 * try { ps = connect.prepareStatement("INSERT INTO m3data " +
			 * "(suggestion,deduceInfo) VALUES" + " ( '" +
			 * suggestionIterator.next() + "','" + deduceInfoIterator.next() +
			 * "')");
			 * 
			 * ps.executeUpdate();
			 * 
